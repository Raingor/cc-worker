...
function renderGreeting(data) {
  ...
  el.innerHTML =
    ...
    '</div>' +
    '<div class="oa-greeting-bear">' + randomBearImg(60, 14) + '</div>' +
    '<div class="oa-greeting-cycle">' +
    '  <svg class="oa-cycle-ring" viewBox="0 0 52 52">' +
    '    <circle cx="26" cy="26" r="22" fill="none" stroke="rgba(255,255,255,.15)" stroke-width="3"/>' +
    ...
    '    <text x="26" y="26" text-anchor="middle" dominant-baseline="central" fill="#fff" font-size="12" font-weight="600">' + pct + '%</text>' +
    '  </svg>' +
    '</div>';
  return el;
}
...
function updateProgressRing() {
  ...
  const circumference = 2 * Math.PI * 22;
  const offset = circumference - (pct / 100) * circumference;
  const ring = document.querySelector('.oa-cycle-ring');
  if (ring) {
