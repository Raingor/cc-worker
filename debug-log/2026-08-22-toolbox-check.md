...
## 2026-08-23：桌面顶部菜单图标对齐修复
- 现象：顶部菜单图标位于 20px 圆角底框下方，没有与底框中心对齐。
- 原因：`.nav-item` 的 `line-height: 36px` 被伪元素 `::before` 继承；即使使用 grid 居中，字符行盒仍按 36px 计算，导致字形视觉中心向下偏移。
- 修复：桌面模式下将 `.nav-item::before` 改为 flex 居中，明确设置 `line-height: 1`、固定 flex 尺寸，并增加 `translateY(-.5px)` 的光学对齐。
- 缓存版本：`mac-layout.css` 从 v8 更新为 v9。

## 2026-08-23：桌面主菜单与下拉菜单交互拆分
- 现象：点击“工具箱”等主菜单切换页面时，下拉菜单也会同时自动展开，遮挡内容并增加一次关闭操作。
- 原因：`app.js` 负责主菜单页面切换，`layout-switcher.js` 又对同一次 `.nav-item` 点击执行下拉框展开，两个监听器表达了不同操作意图。
- 修复：
  - 点击菜单图标或文字：只切换对应工作区，并保持下拉菜单关闭。
  - 点击右侧小箭头：只展开/关闭子菜单，不重复切换工作区。
  - 点击页面其他位置：关闭已经展开的子菜单。
